...
def main():
    ...
    for i, comment in enumerate(sorted_comments[:5], 1):
        badges = f" [{', '.join(comment['author_badges'])}]" if comment['author_badges'] else ""
    ...
    if top_level_comments:
        avg_likes = sum(c['like_count'] for c in top_level_comments) / len(top_level_comments)
        avg_replies = sum(c['reply_count'] for c in top_level_comments) / len(top_level_comments)
        ...
        high_engagement = [c for c in top_level_comments if c['like_count'] > avg_likes * 2]
    ...
    print(f"• Most engaging content creators")
    print(f"• Topics that generate discussion")
    print(f"• Comment threads worth following")
    print(f"• Community engagement patterns")
