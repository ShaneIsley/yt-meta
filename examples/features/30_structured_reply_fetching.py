...
def main():
    ...
    comments_with_tokens = list(client.get_video_comments_with_reply_tokens(
        video_url,
        limit=50,
        sort_by="top"
    ))
    ...
    for i, comment in enumerate(comments_with_tokens, 1):
        ...
        if 'reply_continuation_token' in comment:
            ...
            comments_with_replies.append({
                'comment': comment,
                'token': comment['reply_continuation_token']
            })
        else:
            print("  ❌ No replies")
    ...
    total_replies = sum(len(list(client.get_comment_replies(video_url, item['token'], limit=10)))
